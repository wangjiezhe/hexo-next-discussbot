...
hexo.extend.filter.register('theme_inject', injects => {
  ...
  {% if page.comments %}
  <div class="comments">
  <script async src="https://comments.app/js/widget.js?2" data-comments-app-website="${config.siteid}" data-limit="${config.number}" data-color="${config.color}" data-dislikes="${config.dislikes}" data-outlined="${config.outlined}"></script>
  </div>
  {% endif %}
  `);
